...
const suitColor = computed(() => {
  ...
  if (!props.activeCard) return '';
  return ['Hearts', 'Diamonds'].includes(props.activeCard.suit)
    ? 'text-nott-red'
    : 'text-nott-white';
});
  ...
  <div class="flex items-center gap-3">
    <div v-if="activeCard || selectedJoker" class="flex items-center gap-3">
      ...
      <div class="flex flex-col">
        <Text variant="micro" color="muted">{{ content.currentThreat.label }}</Text>
        <Text variant="body" leading="none"><strong>{{ cardName }}</strong></Text>
      </div>
    </div>
    <div v-else>
        <Text variant="body" color="muted"><em>{{ content.currentThreat.noThreat }}</em></Text>
    </div>
  </div>
</template>
