  ...
  for (const key in s) {
    if (Object.hasOwn(s, key)) {
      if (Object.hasOwn(t, key)) {
        result[key] = deepMerge(t[key], s[key]);
      } else {
        result[key] = s[key];
      }
    }
...
export interface WelcomeScreenRaw {
  title: string;
  intro: string[];
  infoGrid: {
    label: string;
...
export function getGameSetupContent(playsetId?: string | null): GameSetupContent {
  const data = loadData<GameSetupContent>('gameSetup.json', playsetId);
...
interface ActSetupDataRaw {
  title: string;
  quote: string;
  sections?: ActSetupSectionRaw[];
  buttonText: string;
}

export function getActSetupContent(
  act: number | string,
  playsetId?: string | null
): ActSetupContent | null {
  const data = loadData<Record<string, ActSetupDataRaw>>('actSetup.json', playsetId);
  ...
  return {
    ...
    sections: actData.sections
      ? actData.sections.map((section) => ({
          title: section.title,
          intro: section.intro ? (marked.parseInline(section.intro) as string) : undefined,
          steps: section.steps.map((step) => marked.parseInline(step) as string),
...
export function getWelcomeScreenContent(playsetId?: string | null): WelcomeScreenContent {
  ...
  const infoGrid = data.infoGrid.map((item) => {
...
export interface SceneSetupContent {
  ...
  ui: {
    ...
    promptTitle: string;
    startButton: string;
    unknownCardHint: string;
  };
}

export function getSceneSetupContent(playsetId?: string | null): SceneSetupContent {
  const data = loadData<SceneSetupContent>('sceneSetup.json', playsetId);
...
export function getConversationAndStakesContent(
  playsetId?: string | null
): ConversationAndStakesContent {
  const data = loadData<ConversationAndStakesContent>('conversationAndStakes.json', playsetId);
...
export function getResolutionPhaseContent(playsetId?: string | null): ResolutionPhaseContent {
  const data = loadData<ResolutionPhaseContent>('resolutionPhase.json', playsetId);
...
export function getResolveScenePhaseContent(playsetId?: string | null): ResolveScenePhaseContent {
  const data = loadData<ResolveScenePhaseContent>('resolveScenePhase.json', playsetId);
...
export function getFalloutPhaseContent(playsetId?: string | null): FalloutPhaseContent {
  const data = loadData<FalloutPhaseContent>('falloutPhase.json', playsetId);
  ...
  return {
    ...
    standard: {
      success: {
        ace: data.standard.success.ace
          ? {
              title: data.standard.success.ace.title,
              body: marked.parseInline(data.standard.success.ace.body) as string,
            }
          : undefined,
        trophy: {
    ...
    breakingPoint: {
      title: data.breakingPoint.title,
      body: marked.parseInline(data.breakingPoint.body) as string,
    },
  };
}

export interface TrophySetupContent {
  title: string;
  ingress: string;
}

export function getTrophySetupContent(playsetId?: string | null): TrophySetupContent {
  const data = loadData<TrophySetupContent>('trophySetup.json', playsetId);
  return {
    title: data.title,
    ingress: marked.parseInline(data.ingress) as string,
  };
...
export interface WinScreenContent {
  title: string;
  subtitle: string;
  body: string;
  buttonText: string;
...
export function getWinScreenContent(playsetId?: string | null): WinScreenContent {
  return loadData<WinScreenContent>('winScreen.json', playsetId);
}

export interface LoseScreenContent {
  title: string;
  subtitle: string;
  quote: string;
  buttonText: string;
...
export function getLoseScreenContent(playsetId?: string | null): LoseScreenContent {
  return loadData<LoseScreenContent>('loseScreen.json', playsetId);
}

export interface StrikeAssignmentContent {
  title: string;
  subtitle: string;
  pendingLabel: string;
...
export function getStrikeAssignmentContent(playsetId?: string | null): StrikeAssignmentContent {
  return loadData<StrikeAssignmentContent>('strikeAssignment.json', playsetId);
}

export interface LivePlayHeaderContent {
  resetButton: string;
  act3CountdownLabel: string;
  trophyPile: {
    label: string;
    unknown: string;
  };
...
export function getLivePlayHeaderContent(playsetId?: string | null): LivePlayHeaderContent {
  return loadData<LivePlayHeaderContent>('livePlayHeader.json', playsetId);
}
...
export interface UiLabelsContent {
  ...
  promptMatrix: {
    ...
    recurringNightmare: string;
    selectRankHint: string;
  };
  unknownCard: {
    hint: string;
  };
...
export function getUiLabelsContent(playsetId?: string | null): UiLabelsContent {
  return loadData<UiLabelsContent>('uiLabels.json', playsetId);
}
...
export function getHeaderContent(playsetId?: string | null): HeaderContent {
  return loadData<HeaderContent>('header.json', playsetId);
}
